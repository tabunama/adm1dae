Citation, License and Contact
=============================

Citation
--------

adm1dae Python package (2026).
Taher Abunama.

For software context and the aqueous-phase chemistry framework integrated with ADM1/BSM2, see Flores-Alsina et al. (2015).

Upstream license agreement and attribution
------------------------------------------

The ``adm1dae`` package is based on an ADM1 / BSM2 implementation with algebraic pH and H2 solvers.

MATLAB/Simulink/C original development and modification lineage includes:

- Christian Rosen
- Darko Vrecko
- Ulf Jeppsson
- Xavier Flores-Alsina
- Krist V. Gernaey

Contact
-------

- Dr Taher Abunama (tabunama@gmail.com)
